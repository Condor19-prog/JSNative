...
import {
  ...
  selectCurrencies, selectCurrencyReducer,
...
const CurrencyEContainer: React.FunctionComponent = () => {
  ...
  const {currencies,currentCurrency,isBuying,amountOfBYN,amountOfCurrency} = useSelector(selectCurrencyReducer)
  ...
  const changeCurrencyField = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isFinite(+value)) return; // для проверки конечного числа. isFinite исследует число в своем параметре
    if (e.currentTarget.dataset.currency) { //dataset - атрибут HTMLElement.dataset
